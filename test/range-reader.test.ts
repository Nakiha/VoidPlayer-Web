...
test('Synchronous FFmpeg AVIO requests bounded chunks through the async bridge and preserves IO errors', { timeout: 30000 }, async t => {
  ...
  for (const failing of [false, true]) {
    ...
    try {
      ...
      const frame = await rpc.call<ArrayBuffer>('extract', { ctx: init.ctx, index: 1 });
      assert.equal(frame.byteLength, 320 * 180 * 4); assert.ok(new Set(new Uint8Array(frame).subarray(0, 4096)).size > 2);
