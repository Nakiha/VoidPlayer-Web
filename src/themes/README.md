...
子轨道列为名称、偏移、时间轴和关闭操作。表头名称列右边的分隔条可拖动调整所有行的文件名宽度；双击恢复默认，方向键微调，Home/End 到边界，拖动中 Escape 撤销。标注面板在其左侧，默认折叠为 `--dock-rail-width`（40px）图标列；展开使用 `--dock-tools-width`（默认与最小宽度 160px 相同），可拖动调宽。悬浮或键盘聚焦标注显示时间、备注、作者及可用缩略图，展开列表复用同一内容。浮层使用 `--annotation-preview-width`、`--preview-fill` 和 `--popover-shadow`。
...
标注卡片、折叠列表、子轨道时间轴和悬停预览共用 `markIdentity(id)` 的固定 FNV-1a 映射（8 色 × 五边形/方形/三角形/菱形/圆形，共 40 种组合）。映射只依赖 ID，无额外存储、不做避重；文字编辑、轨道排序和折叠不会改变身份外观，有限组合不保证全局唯一。标记轮廓为 2 CSS px，悬停使用本色加深填充和轻微放大，时间轴与列表对应项联动强调，不再显示灰色按钮块。展开列表保留缩略图、时间、备注及垃圾桶删除操作，双击标注可进入编辑，采用与片源列表一致的轻斑马底色和无边框通栏布局；悬停只用 7% 标记色铺底。不为内容行添加阴影或磨砂，避免重复的玻璃层和绘制开销。收起面板、添加标注和删除按钮沿同一右侧列对齐，共用 28px 命中区、18px Regular 图标及相同内边距；折叠栏保留标记身份图标。
