      ...
      <section class="comparison" aria-label="视频对比">
        <div class="viewport-surface"><div class="screens">${SLOTS.map(slot => `
          <article class="video-card" data-slot="${slot}"><div class="card-heading" data-track-drag="${slot}">
            <button class="track-identity" data-inspect="${slot}" data-drag-surface="${slot}" aria-label="检查轨道 ${slot}" data-tooltip="展开或收起轨道详情；拖动排序"><span class="slot slot-${slot}">${slot}</span><span id="name-${slot}" class="filename"></span></button>
            ...
            </div><input id="file-${slot}" type="file" accept="video/*,.mkv,.mov,.mp4,.webm,.ts,.avi" aria-label="打开视频 ${slot}" hidden></div>
          <div class="frame-stage" id="stage-${slot}"><canvas id="grid-${slot}" class="pixel-grid" aria-hidden="true" hidden></canvas><span id="grid-label-${slot}" class="pixel-grid-label" hidden></span><div class="empty" id="empty-${slot}"><label class="empty-open" for="file-${slot}">${icon('filePlus')}<span>添加视频</span></label><span class="empty-hint">或将文件拖入这里</span>${slot === 'A' ? `<section class="start-library" aria-label="启动片源"><header><div class="segmented"><button data-start-tab="available" aria-pressed="true">可用片源</button><button data-start-tab="recent" aria-pressed="false">最近打开</button></div><button id="start-library-more">全部片源</button></header><div id="start-library-list"></div><p id="start-library-status" class="muted"></p></section>` : ''}</div><div id="image-${slot}" class="image-wrap" hidden><canvas id="canvas-${slot}" aria-label="视频 ${slot} 当前解码画面"></canvas><canvas id="annotations-${slot}" class="frame-annotations" aria-hidden="true"></canvas><canvas id="drawing-${slot}" class="drawing-layer" aria-label="在视频 ${slot} 上绘制标注" hidden></canvas><svg id="region-${slot}" viewBox="0 0 1 1" preserveAspectRatio="none" aria-hidden="true"><rect x="0" y="0" width="0" height="0"/></svg></div><button id="recover-${slot}" class="recover-view" aria-label="居中轨道 ${slot} 画面，保留倍率" hidden>${icon('center')}画面已移出 · 居中</button></div>
        ...
        <section class="transport glass" aria-label="共用播放控制" hidden>
          <div class="transport-actions" role="group" aria-label="播放功能">
            <div class="play-buttons"><button class="icon-button" id="previous" title="上一帧，左方向键" aria-label="上一帧" disabled>${icon('previous')}</button><button class="icon-button" id="play" aria-label="播放" disabled>${icon('play')}</button><button class="icon-button" id="next" title="下一帧，右方向键" aria-label="下一帧" disabled>${icon('next')}</button></div>
            <div class="transport-time"><input id="position" class="time-input" type="text" aria-label="定位时间" autocomplete="off" spellcheck="false" value="00:00.000" disabled><span class="duration">/ <span id="duration">00:00.000</span></span></div>
          <div class="timeline-control"><input id="timeline" type="range" min="0" max="1" step="1" value="0" aria-label="共用时间轴，微秒" disabled><output id="timeline-preview" class="seek-preview" hidden></output></div>
...
<section id="annotation-toolbar" class="annotation-toolbar" aria-label="视图内标注" hidden>
    <form id="mark-form"><div class="drawing-tools" role="toolbar" aria-label="画笔工具">${([['pen','画笔'],['ellipse','圈选'],['rect','矩形'],['line','线条'],['text','文字']] as const).map(([tool,label]) => `<button type="button" data-drawing-tool="${tool}" class="icon-button" aria-label="${label}" data-tooltip="${label}" aria-pressed="${tool === 'pen'}">${icon(tool)}</button>`).join('')}<button type="button" id="drawing-undo" class="icon-button" aria-label="撤销上一笔" data-tooltip="撤销上一笔">${icon('undo')}</button></div>
    <textarea id="note" aria-label="备注" rows="1" maxlength="2000" placeholder="备注（可选）"></textarea><button id="add-mark" type="submit">保存标注</button><button id="mark-close" type="button" class="icon-button" aria-label="取消标注" data-tooltip="取消本次绘制">${icon('close')}</button><span id="mark-frame-label" class="sr-only"></span></form><p id="drawing-error" role="alert" hidden></p></section>
      </section>
      <aside id="sources-panel" class="side-panel sources-panel glass" aria-label="片源" hidden>
        ...
        <footer class="panel-foot"><button id="sources-import">${icon('filePlus')}添加片源</button><input id="source-files" type="file" multiple accept="video/*,.mkv,.mov,.mp4,.webm,.ts,.avi" hidden></footer>
      </aside>
  ...
  <input id="drawing-text" aria-label="画面文字" placeholder="输入文字，回车完成" maxlength="2000" hidden>

  <dialog id="help"><header class="dialog-heading"><h2>快捷键与说明</h2><button id="help-close" class="icon-button" aria-label="关闭说明">${icon('close')}</button></header><p>← / → 逐帧 · 空格播放 / 暂停 · M 切换并排 / 分屏。</p><p>滚轮或捏合缩放；右键拖动或双指滚动平移。暂停后可框选并标注。</p><p>本地文件不上传。标注保留在本次会话，关闭前请导出。最近片源保留文件信息，重新打开本地文件需要再次选择。</p><p>当前静音播放。HDR 来源会单独标识，浏览器最终色彩输出尚未验证，WASM 路径为 SDR。</p><details><summary>呈现诊断</summary><p class="evidence"><span id="alignment"></span><span id="decode"></span></p><button id="benchmark">检查当前视频播放性能</button></details></dialog>`;
}
