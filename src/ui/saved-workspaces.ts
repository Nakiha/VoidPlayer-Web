...
export function savedWorkspaceShell() {
  return `<h4 class="settings-section-title">服务器工作区</h4><p class="settings-caption">主动保存到当前服务器，默认仅自己与管理员可访问。保存视频引用和标注，不上传视频文件。</p>
...
export function installSavedWorkspaces(options: { signal: AbortSignal; snapshot(): WorkspaceFile; open(document: WorkspaceFile): Promise<boolean>; canSave(): boolean; report(error: Error): void }) {
  ...
  $('delete-confirm').onclick = () => void act(async () => { if (!deleting) return; await client.remove(deleting); if (binding?.id === deleting.id) binding = undefined; deleting = null; $('delete').hidden = true; message('已从服务器删除，当前会话保留。'); await list(); });
  const settings = document.getElementById('settings')!;
  settings.addEventListener('settings-pane-change', event => { if ((event as CustomEvent).detail === 'workspace') void act(async () => {
    const health = await fetch('/api/health', { signal: options.signal }).then(r => r.json()); available = !!health.capabilities?.workspaces; controls();
