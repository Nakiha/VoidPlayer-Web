...
export function presentationColor(kind: 'video-sample' | 'rgba8', description: FrameDescription) {
  ...
  return { color: description.color, sourceColor: description.sourceColor ?? null, hdr, sourceHdr,
    ...
    conversion: canvasConversion ? 'canvas2d-srgb' : unsupportedHdr ? 'rgba8-hdr-unmanaged' : kind === 'rgba8' ? 'rgba8-upload' : 'browser-default' };
}
