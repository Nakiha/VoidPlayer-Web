...
try {
  ...
  assert.match(await page.locator('#library-root').innerText(),/实验一/);
  await page.waitForFunction(()=>document.querySelectorAll('#source-list .source-actions').length===1);
  assert.match(await page.locator('#source-list').innerText(),/clip-069/);
  await page.locator('[data-source-tab=recent]').click();await page.locator('#source-search').fill('最近搜索');
  ...
  assert.equal(await page.locator('[data-source-tab=recent]').getAttribute('aria-pressed'),'true');
  assert.equal(await page.locator('#source-search').inputValue(),'最近搜索');
  assert.equal((await page.evaluate(()=>window.voidPlayer.exportWorkspace())).layout.sources.search,'clip-069');
