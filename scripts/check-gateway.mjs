// Optional local TLS/auth integration smoke. CADDY_BIN must name Caddy 2.11.4.
...
const password = randomBytes(18).toString('hex'), token = randomBytes(32).toString('hex');
const hash = execFileSync(binary, ['hash-password'], { input: password + '\n', encoding: 'utf8', stdio: ['pipe','pipe','pipe'] }).trim();
await writeFile(path.join(directory, 'users.caddy'), `qa.tester ${hash}\n`);
const original = await readFile('deploy/Caddyfile', 'utf8');
const config = '{\n admin off\n auto_https disable_redirects\n skip_install_trust\n}\n' + original.replace('{$VOIDPLAYER_SITE} {', '{$VOIDPLAYER_SITE} {\n bind 127.0.0.1').replace('/etc/caddy/users.caddy', path.join(directory, 'users.caddy')).replace('app:5180', `127.0.0.1:${apiPort}`);
...
try {
  ...
  await wait(async () => (await request('/')).status === 401);
  const headers = { authorization: 'Basic ' + Buffer.from(`qa.tester:${password}`).toString('base64'), 'x-voidplayer-user':'forged' };
  assert.equal((await request('/', {authorization:'Basic '+Buffer.from('qa.tester:wrong').toString('base64')})).status,401);
  const health = await request('/api/health',headers);
  assert.equal(JSON.parse(health.body).actor.id,'qa.tester');
  ...
  assert.equal((await fetch(`http://127.0.0.1:${apiPort}/api/library`,{headers:{'x-voidplayer-user':'forged'}})).status,401);
  await pause(); assert.ok(appOutput.includes('"actorId":"qa.tester"')); assert.ok(!appOutput.includes(token));
  console.log('PASS: trusted TLS, individual login, spoof-resistant actor forwarding, protected media Range, isolation headers, attributed access audit.');
} finally {
