...
try{
 ...
 const read=async()=>JSON.parse(await page.locator('.log-json').inputValue());
 ...
 await page.waitForFunction(id=>JSON.parse(document.querySelector('.log-json').value).sessionId===id,session);
 ...
 await page.waitForFunction(id=>JSON.parse(document.querySelector('.log-json').value).sessionId===id,current);
 assert.equal(await page.locator('#log-description').inputValue(),'新会话描述');
 assert.deepEqual(errors,[]);console.log(`PASS ${name}: description preview/download/copy/upload, failed-upload retention, session isolation, no automatic upload, desktop themes and mobile layout`);
