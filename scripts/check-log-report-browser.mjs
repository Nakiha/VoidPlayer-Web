...
try{
 ...
 const singleLineHeight = await page.locator('#log-description').evaluate(el => el.getBoundingClientRect().height);
 assert.equal(await page.locator('.log-settings-card [data-action=refresh], .log-description-field label').count(),0,'upload card has no manual refresh or description label');
 assert.equal(await page.locator('.log-settings-card .log-session-row').evaluate(el=>getComputedStyle(el).borderBottomWidth),'0px');
 assert.equal(await page.locator('.log-settings-card .log-send-footer').evaluate(el=>getComputedStyle(el).borderTopWidth),'0px');
 assert.ok(singleLineHeight < 60, 'empty description starts at one line');
 assert.equal(await page.locator('#log-description').evaluate(el => getComputedStyle(el).borderTopWidth), '0px');
 assert.equal(await page.locator('#log-description').evaluate(el => getComputedStyle(el).backgroundColor), await page.locator('.log-json').evaluate(el => getComputedStyle(el).backgroundColor), 'description uses the log preview surface');
 assert.equal(await page.locator('.log-report').evaluate(el => getComputedStyle(el).backgroundColor === getComputedStyle(el.closest('[role=tabpanel]')).backgroundColor), false, 'local log report has its own surface');
 assert.equal(await page.locator('.log-report').evaluate(el => parseFloat(getComputedStyle(el).paddingLeft)), 16, 'local log report uses card padding');
 assert.equal(await page.locator('.log-report-toolbar').evaluate(el => getComputedStyle(el).borderBottomWidth), '0px', 'log preview has no toolbar divider');
 assert.notEqual(await page.locator('.log-json').evaluate(el => getComputedStyle(el).backgroundColor), await page.locator('.log-report').evaluate(el => getComputedStyle(el).backgroundColor), 'log text uses the darker outer surface');
 assert.equal(await page.locator('.log-json').evaluate(el => getComputedStyle(el).backgroundColor), await page.locator('#settings').evaluate(el => getComputedStyle(el).backgroundColor), 'log text matches the background behind the local logs heading');
 assert.deepEqual(await page.locator('.log-preview-status').evaluate(el => ({ color: getComputedStyle(el).color, size: getComputedStyle(el).fontSize })), await page.locator('.log-report-toolbar [data-action=copy]').evaluate(el => ({ color: getComputedStyle(el).color, size: getComputedStyle(el).fontSize })), 'page status matches the action text');
 assert.equal(await page.locator('.log-preview-hint').count(),0);
 ...
 assert.ok(await page.locator('#log-description').evaluate(el => el.getBoundingClientRect().height) > singleLineHeight, 'description grows with text');
 ...
 await page.evaluate(()=>window.dispatchEvent(new Event('focus')));await page.waitForFunction(()=>document.querySelector('.log-panel').getAttribute('aria-busy')==='false');
 assert.equal((await read()).report.description,description);
 ...
 await page.evaluate(()=>Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async text=>{window.copiedReport=text;window.copiedItems??=[];window.copiedItems.push(text);}}}));
 ...
 await page.locator('#settings .toast-error').filter({hasText:'无法连接'}).waitFor();
 assert.equal(await page.locator('.log-feedback').isVisible(),false,'upload failure does not insert a feedback panel');
 ...
 await page.locator('#settings .toast').filter({hasText:'文件名已复制'}).waitFor();
 assert.equal(await page.locator('.log-feedback').isVisible(),false,'upload success does not insert a feedback panel');
 const files=await readdir(logsDir);assert.equal(files.length,1);
 assert.equal(await page.evaluate(()=>window.copiedItems.at(-1)),files[0],'successful upload copies the server filename');
 ...
 await page.locator('.log-panel [data-action=upload]').click();
 await page.locator('#settings .toast-error').filter({hasText:'无法自动复制文件名'}).waitFor();
 assert.equal(await page.locator('.log-feedback').isVisible(),false,'clipboard failure keeps the filename in a toast');
 await page.locator('#log-description').fill('');assert.equal(await page.locator('#log-description').evaluate(el => el.getBoundingClientRect().height), singleLineHeight, 'description shrinks after clearing');assert.ok(!('report' in await read()));
