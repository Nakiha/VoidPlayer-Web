...
if (process.argv.length > 3 || (mode && !['--functional-only', '--benchmark-only'].includes(mode))) {
  throw new Error('Usage: node scripts/check-http-playback.mjs [--functional-only|--benchmark-only]');
}
...
try {
  ...
  if (functional) await writeFile(path.join(root, '.run/playback-reports/repeated-playback.json'), JSON.stringify(reports, null, 2));
  await browser?.close();
  untrust?.();
