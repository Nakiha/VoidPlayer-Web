...
const env = { ...process.env, VOIDPLAYER_SITE: 'https://localhost', VOIDPLAYER_MEDIA_DIR: media, VOIDPLAYER_ADMIN_USERS: 'qa.tester' };
...
// Change only test host ports; keep shipped HTTPS, read-only mounts, images and health checks.
...
try {
  ...
  const auth = { cookie: initial.headers['set-cookie'][0].split(';')[0] };
  const selected = await get('/api/identity', { ...auth, origin: `https://localhost:${port}`, 'content-type': 'application/json', 'x-voidplayer-action': 'identity' }, true, 'POST', JSON.stringify({ name: 'qa.tester' }));
  assert.equal(selected.status, 200); const actor = JSON.parse(selected.body).actor;
  ...
  assert.equal(JSON.parse((await get('/api/health', auth)).body).actor.id, actor.id);
  ...
  const savedWorkspace = await verifySavedWorkspaces(workspaceTransport, listing.entries[0], `https://localhost:${port}/`, actor.id);
  ...
  assert.ok(!Object.values(app.NetworkSettings.Ports).some(Boolean), 'Only the HTTPS gateway may publish ports');
  ...
  assert.equal(JSON.parse((await get('/api/admin/status', auth)).body).identity.id, actor.id);
  ...
  assert.ok(logs.includes(`"actorId":"${actor.id}"`));
  console.log('PASS shipped Docker deployment: no Node/Bun, non-root/read-only runtime, verified TLS, automatic identity and trusted username selection, page/WASM headers, four concurrent media ranges, four bounded measurements through TLS, admin identity and writable configuration on /data, persistent configuration/index/versioned workspaces and CA after restart');
} catch (error) {
