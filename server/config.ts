...
export interface ServiceConfig {
  mediaRoots: string[]; host: string; port: number; devPort: number;
...
export async function loadConfig(args: string[], mode: 'dev' | 'production', cwd = process.cwd(), paths: { configFile?: string; staticDir?: string; logsDir?: string } = {}): Promise<ServiceConfig> {
  ...
  if (!Array.isArray(config.mediaRoots) || !config.mediaRoots.length || config.mediaRoots.some(r => typeof r !== 'string' || !r.trim())) throw new Error('请在配置中设置 mediaRoots，或指定 --folder 白名单目录。');
  ...
  config.mediaRoots = config.mediaRoots.map(r => path.resolve(path.dirname(file), r));
