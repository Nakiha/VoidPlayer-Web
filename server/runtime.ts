...
export async function validateServiceConfig(config: ServiceConfig, requireStatic = true) {
...
export async function startService(config: ServiceConfig, requireStatic = true) {
  const { staticOk, proxyToken } = await validateServiceConfig(config, requireStatic);
