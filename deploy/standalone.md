...
启用 HTTPS 后，主端口同时识别 HTTP 与 HTTPS，例如 `http://192.168.1.20:5180/` 和 `https://192.168.1.20:5180/` 都可访问。普通 HTTP 请求不会再因为误入 TLS 端口而得到空响应。额外保留 `http://192.168.1.20:5181/` 作为兼容入口（默认主端口加 1；65535 时使用 65534），提供同样的网页与 API。可用 `--http-port`（配置 `httpPort`）更换，或 `--no-http-guide`（`httpPort: null`）关闭额外入口；主端口的 HTTP 兼容不受影响。

远程 HTTP 播放器入口先以无凭据的 HTTPS 探测检查浏览器连接是否可用。成功后在当前标签页进入 HTTPS，并保留查询参数和 hash；失败转到证书设置，保留原目标，可在安装后重新检查。该探测不读取系统证书列表；失败也可能由网络、地址不匹配或服务故障引起。直接进入 `/connection` 仍可下载证书，不会自动跳离。

`/admin`（包括所有后台面板）、`/llms.txt`、媒体查询/下载及其他后端 API 继续响应 HTTP，不统一重定向到 HTTPS。写操作仍要求原有的同源请求和操作标记。`localhost` 的 HTTP 本身满足浏览器安全上下文条件，仍可直接进入播放器。
...
媒体索引当前为 SQLite schema 4（0.2.0 新增缩略图表），工作区为 schema 3。回退旧程序时使用升级前的完整数据备份；不要把保存用户和评审的 `workspaces.sqlite` 当缓存删除。视频原文件不在数据备份中。
...
本工具面向可信内网。用户名表示内容归属，不是登录认证；访问者可选择已有身份，并使用管理接口。工作区按用户归属展示不构成安全的多租户隔离。对外部署应在上游配置认证和访问限制，勿直接暴露服务端口。
