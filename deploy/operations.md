...
当前预览版尚无索引数据库迁移；引入索引后会随版本记录兼容性和迁移策略。媒体原文件由存储系统备份，数据目录和工作区文件并不包含视频副本。备份中的账号、令牌、私有 CA 私钥和诊断记录应按原数据的访问权限保管。

容器部署还需备份 Caddy 数据卷以保留已被客户端信任的 CA；操作见 `README.md`。不要在正常升级时使用 `docker compose down -v` 删除该卷。
